...
namespace StartToBike.Controllers
{
    public class TrainingsController : Controller
    {
...

        {
            ...
            {
                TrainingId = game.TrainingId,
                AccountId = account.AccountId,
            };

            

            var exists = db.TrainingAccounts.Where(g => g.TrainingId == game.TrainingId).Where(g => g.AccountId == account.AccountId).Any();


            if (exists)
            {
                return RedirectToAction("Index", new { error = "You already joined the Training!" });
            }
            else
            {
